...
/* Custom Panels */
panorama::IUIPanel* GUI::deephookMain = nullptr;

// Javascript to force root panel to have our child and raise it.
const char *cuckProtocol =
        "var parentPanel = $.GetContextPanel();\n"
                "var deephook = parentPanel.FindChild(\"DeephookMain\");\n"
                "$.Msg(\"Loading Panel: \" + deephook.id);"
                "deephook.BLoadLayoutFromString( \"%s\", false, false);\n";
...
static void SetupAndCheckPanels()
{
    ...
    if( !GUI::deephookMain ){
        cvar->ConsoleDPrintf("Creating Deephook panel...\n");
        ...
        GUI::deephookMain = panoramaEngine->AccessUIEngine()->CreatePanel(&type, "DeephookMain", root)->panel;
    ...
    GUI::deephookMain->SetParent( root );
}

void GUI::ToggleUI()
{
    SetupAndCheckPanels();
    if( !panoramaEngine->AccessUIEngine()->IsValidPanelPointer(GUI::deephookMain) ){
        cvar->ConsoleDPrintf("[GUI::ToggleUI] - Something is wrong with our Deephook Panel Pointer(%p)\n", (void*)GUI::deephookMain);
        return;
    }
    if( !GUI::deephookMain->HasBeenLayedOut() ){
        cvar->ConsoleDPrintf("[GUI::ToggleUI] - Deephook Panel not layed out yet. Try again.\n");
        return;
    }

    GUI::deephookMain->SetVisible(!GUI::deephookMain->IsVisible());
}
