...
void NameChanger::BeginFrame(float frameTime)
{
	...
	if (changes >= 5)
	{
		switch (NameChanger::type)
		{
			case NC_Type::NC_NORMAL:
				SetName(Util::PadStringRight(XORSTR("\230DeepHook"), strlen(XORSTR("\230DeepHook")) + Util::RandomInt(10, 50)));
				break;
	...
	SetName(Util::PadStringRight(XORSTR("DeepHook"), strlen(XORSTR("DeepHook")) + changes));
}
