...
int ImGui::Bezier(const char *label, float P[5], const bool& showPreset, const bool& showPreview)
{
        // visuals
        ...
        static struct { const char *name; float points[4]; } presets [] = {
                { XORSTR("Linear"), 0.000f, 0.000f, 1.000f, 1.000f },

                { XORSTR("In Sine"), 0.470f, 0.000f, 0.745f, 0.715f },
                { XORSTR("In Quad"), 0.550f, 0.085f, 0.680f, 0.530f },
                { XORSTR("In Cubic"), 0.550f, 0.055f, 0.675f, 0.190f },
                { XORSTR("In Quart"), 0.895f, 0.030f, 0.685f, 0.220f },
                { XORSTR("In Quint"), 0.755f, 0.050f, 0.855f, 0.060f },
                { XORSTR("In Expo"), 0.950f, 0.050f, 0.795f, 0.035f },
                { XORSTR("In Circ"), 0.600f, 0.040f, 0.980f, 0.335f },
                { XORSTR("In Back"), 0.600f, -0.28f, 0.735f, 0.045f },

                { XORSTR("Out Sine"), 0.390f, 0.575f, 0.565f, 1.000f },
                { XORSTR("Out Quad"), 0.250f, 0.460f, 0.450f, 0.940f },
                { XORSTR("Out Cubic"), 0.215f, 0.610f, 0.355f, 1.000f },
                { XORSTR("Out Quart"), 0.165f, 0.840f, 0.440f, 1.000f },
                { XORSTR("Out Quint"), 0.230f, 1.000f, 0.320f, 1.000f },
                { XORSTR("Out Expo"), 0.190f, 1.000f, 0.220f, 1.000f },
                { XORSTR("Out Circ"), 0.075f, 0.820f, 0.165f, 1.000f },
                { XORSTR("Out Back"), 0.175f, 0.885f, 0.320f, 1.275f },

                { XORSTR("InOut Sine"), 0.445f, 0.050f, 0.550f, 0.950f },
                { XORSTR("InOut Quad"), 0.455f, 0.030f, 0.515f, 0.955f },
                { XORSTR("InOut Cubic"), 0.645f, 0.045f, 0.355f, 1.000f },
                { XORSTR("InOut Quart"), 0.770f, 0.000f, 0.175f, 1.000f },
                { XORSTR("InOut Quint"), 0.860f, 0.000f, 0.070f, 1.000f },
                { XORSTR("InOut Expo"), 1.000f, 0.000f, 0.000f, 1.000f },
                { XORSTR("InOut Circ"), 0.785f, 0.135f, 0.150f, 0.860f },
                { XORSTR("InOut Back"), 0.680f, -0.55f, 0.265f, 1.550f },
        ...
        if (showPreset)
        {
                ImGui::PushID(XORSTR(label));
                if (ImGui::ArrowButton(XORSTR("##lt"), ImGuiDir_Left)) { // ImGui::ArrowButton(ImGui::GetCurrentWindow()->GetID("##lt"), ImGuiDir_Left, ImVec2(0, 0), 0)
                        if (--P[4] >= 0) reload = 1; else ++P[4];
                }
                ImGui::SameLine();

                if (ImGui::Button(XORSTR("Presets")))
                {
                        ImGui::OpenPopup(XORSTR("!Presets"));
                }
                if (ImGui::BeginPopup(XORSTR("!Presets"))) {
                ...
                if (ImGui::ArrowButton(XORSTR("##rt"), ImGuiDir_Right)) { // ImGui::ArrowButton(ImGui::GetCurrentWindow()->GetID("##rt"), ImGuiDir_Right, ImVec2(0, 0), 0)
        ...
        // header and spacing
        int changed = SliderFloat4(XORSTR(label), P, 0, 1, XORSTR("%.3f"), 1.0f);
        ...
        // control points: 2 lines and 2 circles
        {
                ...
                {
                        SetTooltip(XORSTR("(%4.3f, %4.3f)"), P[selected * 2 + 0], P[selected * 2 + 1]);
